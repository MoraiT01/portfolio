...
INDEX_NAME = os.getenv("HANS_BACKEND_OPENSEARCH_STANDARD_INDEX")
VECTOR_INDEX_NAME = os.getenv("HANS_BACKEND_OPENSEARCH_VECTOR_INDEX")
SLIDES_VECTOR_INDEX_NAME = os.getenv("HANS_BACKEND_OPENSEARCH_SLIDES_VECTOR_INDEX")
SUMMARIES_VECTOR_INDEX_NAME = os.getenv("HANS_BACKEND_OPENSEARCH_SUMMARIES_VECTOR_INDEX")
...
class OpensearchConnector(Connector):
    ...
    def insert_lecture_document(self, id, json_data):
        ...
        if self.client is None:
            self.connect()
        try:
            response = self.client.index(index=INDEX_NAME, body=json_data, refresh=True, id=id, request_timeout=60)
    ...
    def vector_search_in_specific_lectures(
        self, query_vector, lecture_ids: List[str], num_results: int, index: str = VECTOR_INDEX_NAME, width_data=False
    ) -> List[Dict[str, Any]]:
        """
        ...
        :param width_data: return data field of vector image
        ...
        try:
            knn_query = {
                ...
                "query": {
                    "script_score": {
                        "query": {
                            "terms": {"lecture_id": lecture_ids}  # Use 'terms' to match any document_id from the list
                        },
                        "script": {
                            "source": "knn_score",
                            "lang": "knn",
            ...
            results = self.create_result_list_from_vector_search_response(response, width_data=width_data)
            return results
        except opensearchpy.exceptions.OpenSearchException as err:
    ...
    @staticmethod
    def create_result_list_from_vector_search_response(response, width_data=False) -> List[Dict[str, Any]]:
        result_list = []
        for hit in response["hits"]["hits"]:
            result = dict()
            ...
            if width_data is True:
                result["data"] = hit["_source"]["data"]
