...
default_args = {
    "owner": "airflow",
    "description": "Cleanup assetdb-temp archive bucket",
...
# A new major version will result in a new hans python file, e.g. "cleanup_archive_v2.py"
# A new TaskGroup which is introduced will increase the minor version, e.g. "cleanup_archive_v1.1.0"
# Fixes will increase the last part of the version number, e.g. "cleanup_archive_v1.0.1"
...
DAG_NAME = "cleanup_archive_v1.0.0"
with DAG(DAG_NAME, default_args=default_args, schedule_interval=None, catchup=False, max_active_runs=1) as dag:

    start = EmptyOperator(task_id=dag.dag_id + "_cleanup_archive_start")

    from modules.operators.storage import op_cleanup_assetdbtemp_archive

    cleanup_task = op_cleanup_assetdbtemp_archive(dag, dag.dag_id, "cleanup_archive")

    end = EmptyOperator(task_id=dag.dag_id + "_cleanup_archive_end")
