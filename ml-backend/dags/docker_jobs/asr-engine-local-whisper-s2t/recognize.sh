...
while getopts hb:c:d:gl:m:u:t:s:x:y:z:v flag
do
    case "${flag}" in
        h) showHelp=1;;
        b) batch_size=$OPTARG;;
...
then
    ...
    if [[ $recognize -eq 1 ]]; then
        ...
        if [ "$language" == "en" ]; then
            ...
            python3 transcribe.py --device "$torch_device" --model "$asrModel" --language "$language" --task "transcribe" --batch-size $batch_size --vad "True" --word-timestamps "True" --input-file "${AIRFLOW_TMP_DIR}/$inputFile" --output-file "${AIRFLOW_TMP_DIR}/$outputFile" & PID=$!
            ...
            python3 transcribe.py --device "$torch_device" --model "$asrModel" --language "de" --task "transcribe" --batch-size $batch_size --vad "True" --word-timestamps "True" --input-file "${AIRFLOW_TMP_DIR}/$inputFile" --output-file "${AIRFLOW_TMP_DIR}/$outputFile" & PID=$!
            echo "Transcription with whisper ongoing"
            while kill -0 $PID 2> /dev/null; do
                echo -n "."
                sleep 30
            ...
            python3 transcribe.py --device "$torch_device" --model "$asrModel" --language "en" --task "translate" --batch-size $batch_size --vad "True" --word-timestamps "True" --input-file "${AIRFLOW_TMP_DIR}/$inputFile" --output-file "${AIRFLOW_TMP_DIR}/$outputFile" & PID=$!
            echo "Transcription with whisper ongoing"
            while kill -0 $PID 2> /dev/null; do
                echo -n "."
                sleep 30
