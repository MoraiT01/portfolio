...
then
    ...
    if [[ $convert -eq 1 ]]; then
        ...
        echo "Converting ${AIRFLOW_TMP_DIR}/$inputFile to HLS with max. $resolution resolution into folder $outputFolder"
        if [[ $verbose -eq 1 ]]; then
            if [[ $singleResPreview -eq 1 ]]; then
                ./create_hls.sh -i "${AIRFLOW_TMP_DIR}/$inputFile" -o "$outputFolder" -r "$resolution" -t $ffmpeg_threads -v -s
            else
                ./create_hls.sh -i "${AIRFLOW_TMP_DIR}/$inputFile" -o "$outputFolder" -r "$resolution" -t $ffmpeg_threads -v
            fi
        else
            if [[ $singleResPreview -eq 1 ]]; then
                ./create_hls.sh -i "${AIRFLOW_TMP_DIR}/$inputFile" -o "$outputFolder" -r "$resolution" -t $ffmpeg_threads -s
            else
                ./create_hls.sh -i "${AIRFLOW_TMP_DIR}/$inputFile" -o "$outputFolder" -r "$resolution" -t $ffmpeg_threads
            fi
        fi
        retVal=$?
        if [ $retVal -ne 0 ]; then
            echo "Error during hls conversion!"
            exit $retVal
        fi
        echo ""
